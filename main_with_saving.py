...
def parse_opt():
    ...
    # For profiling
    parser.add_argument('--timing-data-path', type=str, help='path to timing data file')
    
    return parser.parse_args()


         
def main(timing_data_path: str | Path,
    ...
    Returns:
        Nish
    """
    if timing_data_path is None:
        print("Timing data path not provided, will not work")
        exit()
    
    # Initialize the model and frame processor
    model = YOLO(f"{weights}").to("cuda")
    processor = FrameProcessor(model=model, verbose=verbose, debug=debug, timing_data_path=timing_data_path)
